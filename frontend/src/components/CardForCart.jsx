...
const CardForCart = ({ name, desc, price, image, id }) => {
    ...
    return (
        <div className="max-w-sm  overflow-hidden shadow-sm transform transition duration-500 hover:scale-105 shadow-white font-serif rounded-md w-[22.5vw]" >
            ...
            <div className="px-6 pt-4 pb-2">
                <button className="bg-orange-500 text-white font-bold py-2 px-4 rounded hover:bg-orange-700 transition duration-300" onClick={() => cartList((prevList) => prevList.filter(obj => obj.name !== name))} >
                    Remove
                </button>
            </div>
        </div>
